...
export function aggregateCosts(pairs: PairedCaseResult[]): CostSummary {
  ...
  if (baseline.total_tokens === 0 && candidate.total_tokens > 0) {
    return {
      ...
      model_call_increase: candidate.model_call_count - baseline.model_call_count,
    };
  }
  return {
    baseline,
...
function sumUsage(items: RunUsage[]): RunUsage {
  return items.reduce<RunUsage>((sum, item) => ({
    ...
    model_call_count: sum.model_call_count + item.model_call_count,
