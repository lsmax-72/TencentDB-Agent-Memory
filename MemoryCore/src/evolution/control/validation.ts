...
export async function validateFrozenContent(deps: ValidationDependencies, candidate: EvolutionRecord): Promise<ValidationReport> {
  ...
  if (p.asset_kind === "wiki") return report("NEEDS_EVIDENCE", ["WIKI_FROZEN_VALIDATOR_BRIDGE_REQUIRED"]);
  ...
  if (p.layer === "L1") {
    ...
    const memory = z.object({ content: z.literal(p.after), source_message_ids: z.array(z.string()).min(1) }).safeParse(p.extracted_memory);
  ...
  if (!await deps.canRead(candidate)) throw new EvolutionError(403, "VALIDATION_SOURCE_ACCESS_DENIED");
  return report("PASS", ["CONTENT_CHECKS_PASSED", "SEMANTIC_TRUTH_AND_CONFLICTS_REQUIRE_HUMAN_REVIEW"], {
